...
def main(args):
    ...
    for epoch in epoch_range:
        ...
        center_weight = 0.5  # + 0.5 * np.sin(np.pi * epoch / args.num_iter)
        ...
        if 'edgeloss' in args.center_method:
            ...
            edgeloss = torch.sum(sketches * (1 - edges)) / 1e6 # / torch.sqrt(torch.sum(sketches)) 
        ...
        if 'motionloss' in args.center_method:
            if epoch < args.num_iter // 2:
                motion_weight = 10 ** (-4 + 2 * epoch * 2 / args.num_iter)
            else:
                motion_weight = 10 ** (-2 - 1 * epoch * 2 / args.num_iter)
        ...
        if epoch % args.eval_interval == 0 and epoch >= args.min_eval_iter:
            ...
            with torch.no_grad():
                ...
                for eval_frame in [args.start_frame, args.center_frame, args.end_frame]:
                    ...
                    if 'edgeloss' in args.center_method:
                        edges = renderer.get_edges(batch_frame_indexes)
                        edgeloss += torch.sum(sketches * (1 - edges))  / 1000 # / torch.sum(sketches)
